...
export function Leaderboard({ rows }: { rows: LeaderboardRow[] }) {
  return (
    <>
      ...
      />
      {/*
        Five columns of counts is a table, and stacking it would produce a
        column of unlabelled numbers — so the columns hold at every width and
        the header labels wrap instead. It fits a 390px phone without the page
        or the element scrolling; the counts are short, and it is the labels
        that need the room.
      */}
      <div className="min-w-0">
        <div>
          {/* A column gap only where the columns get tight — at desk width the
              tracks are wide enough that the prototype sets none, and adding
              one there would move every figure. */}
          <div
            className="gap-x-2 md:gap-x-0"
            style={{
              display: "grid",
